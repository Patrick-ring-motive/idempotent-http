  ...
  (() => {
    ...
    for (const Record of [Request, Response, Blob]) {
      ...
      for (const fn of consumables) {
        ...
        Object.defineProperty(Record.prototype[fn], 'name', {
          get: () => fn
        });
      }
      if (_body) {
    ...
    const $cloneStream = (stream) => {
      if (streams.has(stream)) {
        ...
        const $stream = resBody.call(new orgiginalResponse(stream, {
          duplex: 'half'
        }));
    ...
    for (const fn of streamConsumables) {
      ...
      Object.defineProperty(ReadableStream.prototype[fn], 'name', {
        get: () => fn
      });
    }
